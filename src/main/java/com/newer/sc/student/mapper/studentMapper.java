...
//	查看班级考试安排
	...
	@Results(value = { @Result(column = "e_cid", property = "class1", one = @One(select = "findClassById")),
...
//	通过班级ID查班级信息
	@Select("select * from class where cid=#{id}")
	@Results(value = { @Result(column = "c_maid", property = "major", one = @One(select = "findMajor")),
...
//	查询班级的课表
	...
	@Results(value = { @Result(column = "ct_cid", property = "class1", one = @One(select = "findClassById")),
...
//	通过老师id查老师
	...
	@Results(value = { @Result(column = "t_coid", property = "college", one = @One(select = "findCollege")) })
	public Teacher findTeacherById(int id);

//	查询学分
	@Select("select * from credit where cr_sid=#{id}")
	@Results(value = { @Result(column = "cr_gid", property = "grade", one = @One(select = "findGrade")),
...
//	查询分数
	...
	@Results(value = { @Result(column = "g_sid", property = "student", one = @One(select = "findByid")),
...
//  查询已申请补考
	...
	@Results(value = { @Result(column = "es_eid", property = "exam", one = @One(select = "findExam")),
...
//	查询已申请的证书考试
	...
	@Results(value = { @Result(column = "sc_cid", property = "ccie", one = @One(select = "findCcieById")),
...
//	查看已申请的选修课
	...
	@Results(value = { @Result(column = "eles_elid", property = "elective", one = @One(select = "findEleByid")) })
